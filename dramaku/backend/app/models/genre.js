...
const { getTotalGenres } = require('../controllers/genreController');

const Genre = {
    getAll: async () => {
        const res = await pool.query('SELECT * FROM genres ORDER BY updated_at ASC NULLS LAST');
        return res.rows;
    },
    ...
    getPaginatedGenres: async (offset, limit) => {
        try {
            ...
            throw new Error(
                "Failed to get paginated genres: " + error.message
            );
        }
    },
    ...
    searchByGenreName: async (keyword, page = 1, limit = 10) => {
        try {
            ...
            throw new Error("Failed to search genres by genre name or ID: " + error.message);
        }
    },
    ...
    delete: async (id) => {
        try {
            ...
            if (categorizedAsCheck.rowCount > 0) {
                throw new Error(
                    "Genre masih terkait dengan movie."
                );
            }
